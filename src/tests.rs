...
fn it_works_assign_signer() {
	new_test_ext().execute_with(|| {
		...
		let assign_signer_result = Audit::assign_signer(Origin::signed(1), 1, account_id);
		let file = Audit::get_file_by_id(1);
...
fn it_works_assign_signer_do_no_dublicates() {
	new_test_ext().execute_with(|| {
		...
		// Try Dublicate:
		let _ = Audit::assign_signer(Origin::signed(1), 1, account_id);
...
fn it_works_delete_signer() {
	new_test_ext().execute_with(|| {
		...
		let file_with_signer = Audit::get_file_by_id(1);
		let delete_signer_result = Audit::delete_signer(Origin::signed(1), 1, account_id);

		// Check file state after delete
		let file_without_signer = Audit::get_file_by_id(1);
...
fn it_fails_delete_signer_no_signers() {
	new_test_ext().execute_with(|| {
		...
		let delete_signer_result_no_signers = Audit::delete_signer(Origin::signed(1), 1, account_id);

		// Second - try to delete unexisting signer after delete:
		let _ = Audit::assign_signer(Origin::signed(1), 1, account_id);
		let _ = Audit::delete_signer(Origin::signed(1), 1, account_id);
		let delete_signer_result_after_delete = Audit::delete_signer(Origin::signed(1), 1, account_id);
